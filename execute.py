...
st.set_page_config(page_title="Recherche de Visages Similaires", page_icon=":smiley:", layout="wide")
st.markdown(
    """
    <style>
    .stApp {
...
SIMILAR_IMAGES_FOLDER = './images'

# Chargement de la base de signatures
signatures_db = np.load('FaceSignature_db1.npy', allow_pickle=True)
...
def find_similar_faces(target_image, threshold=0.9):
...
def get_similar_images_paths(similar_faces):
    ...
    for name, _ in similar_faces:
        image_path = os.path.join(SIMILAR_IMAGES_FOLDER, name + '.png')  # Supposant que les images sont en format jpg
